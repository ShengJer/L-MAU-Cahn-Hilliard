...
parser.add_argument('-train_data_paths', type=str, default='./data/encoder_data/train_data_300.npz')
parser.add_argument('-valid_data_paths', type=str, default='./data/encoder_data/valid_data_300.npz')
parser.add_argument('-test_data_paths', type=str, default='./data/encoder_data/test_data_300.npz')
parser.add_argument('-test_ms_data_paths', type=str, default='../../High_Dimension_data/microstructure_data/test')

parser.add_argument('-PCA_dir', type=str, default='./data/PCA_model')
parser.add_argument('-PCA_name', type=str, default='pca_300.pkl')
...
parser.add_argument('-save_modelname', type=str, default='model.pt.tar-72000')

# Autoencoder parameters:
parser.add_argument('-Autoencoder_dir', type=str, default='./data/Autoencoder_model')
parser.add_argument('-save_Autoencodername', type=str, default='EncoderDecoder.pt.tar')

parser.add_argument('-batch_size', type=int, default=1)
parser.add_argument('-in_features', type=int, default=300)
...
parser.add_argument('-num_hidden', type=int, default=256) # 64 or 128
...
parser.add_argument('-num_save_samples', type=int, default=2)
...
parser.add_argument('-sampling_start_value', type=float, default=1.0) # probability of starting value
...
def Graph(criterion, idex):
    ...
    plt.savefig(name)
    plt.close()
def train_wrapper(model, pca_model, Automodel):
    begin = 0
    ...
    train_input_handle, val_input_handle = datasets_factory.data_provider(
        ...
        ms_data_path = args.test_ms_data_paths,
    ...
    for itr in range(1, args.max_iterations + 1):
        ...
        if itr % args.test_interval == 0:
            print('Validate:')
            avg, frame_cri = trainer.test(model, pca_model, Automodel, val_input_handle, args, itr)
...
def test_wrapper(model, pca_model, Automodel):
    current = os.getcwd()
    ...
    trainer.test(model, pca_model, Automodel, test_input_handle, args, 'test_result')
...
if __name__ == '__main__':
    ...
    # Autoencoder model
    Autoencodername = os.path.join(args.Autoencoder_dir, args.save_Autoencodername)
    
    
    Automodel = LCA(in_channels=args.img_channel)
    
    Automodel.to(args.device)
    ## load
    checkpoint_path = args.Autoencoder_dir
    filename = os.path.join(checkpoint_path, args.save_Autoencodername)
    print("load Automodel from"+filename)
    stats = torch.load(filename, map_location=args.device)
    Automodel.load_state_dict(stats['model_param'])
    Automodel.eval()
    ...
    if args.is_training:
        ...
        train_wrapper(model, pca_model, Automodel)
        
    else:
        test_wrapper(model, pca_model, Automodel)
