...
def test(model, pca_model, Automodel, test_input_handle, configs, itr):
    ...
    while (test_input_handle.no_batch_left() == False or batch_id ==0):
        ...
        if configs.is_training == 0:
            ...
            orig_ims_np=orig_scalems_np.reshape(-1, configs.in_features) #(BxTL, Fin)
            pred_np=pred_scale_np.reshape(-1, configs.in_features) # (BxOL, Fin)
            
            orig_ims_np=pca_model.inverse_transform(orig_ims_np).reshape(configs.batch_size, configs.total_length, configs.latent_channel, configs.latent_height, configs.latent_width)
            pred_np=pca_model.inverse_transform(pred_np).reshape(configs.batch_size, configs.output_length, configs.latent_channel, configs.latent_height, configs.latent_width)
