...
parser.add_argument('-train_data_paths', type=str, default='./data/encoder_data/train_data_256.npz')
parser.add_argument('-valid_data_paths', type=str, default='./data/encoder_data/valid_data_256.npz')
parser.add_argument('-test_data_paths', type=str, default='./data/encoder_data/test_data_256.npz')
...
parser.add_argument('-save_Autoencodername', type=str, default='EncoderDecoder256x1x1.pt.tar')
...
def Graph(criterion, idex):
    ...
    plt.savefig(name)
    plt.close()
def train_wrapper(model, Automodel):
    begin = 0
    ...
    for itr in range(1, args.max_iterations + 1):
        ...
        if itr % args.test_interval == 0:
            print('Validate:')
            avg, frame_cri = trainer.test(model, Automodel, val_input_handle, args, itr)
...
def test_wrapper(model, Automodel):
    current = os.getcwd()
    ...
    trainer.test(model, Automodel, test_input_handle, args, 'test_result')
...
if __name__ == '__main__':
    ...
    Automodel = HCA(in_channels=args.img_channel)
    ...
    if args.is_training:
        ...
        train_wrapper(model, Automodel)
        
    else:
        test_wrapper(model, Automodel)
